...
impl RC500 {
    ...
    pub fn midi_messages(&mut self, event: RC500Action) -> MidiMessages {
        match event {
            ...
            RC500Action::CurrentChannelLevel(value) => control_change(Control::new(6), value),
            RC500Action::RhythmPattern(dir) => self.patterns.go(&PATTERNS, dir),
            RC500Action::DrumKit(dir) => self.drumkits.go(&DRUMKITS, dir),
        }
    }
}
...
fn control_toggle(control: u8) -> MidiMessages {
    ...
    messages.push(MidiMessage::ControlChange(RC500_CHANNEL, c, MAX_V7));
    messages.push(MidiMessage::ControlChange(RC500_CHANNEL, c, MIN_V7));
    messages
}
